...
/**
 * /ops/proposals/[publicId]/preview — "view as client": the exact sheet the
 * client sees and the PDF prints (one design everywhere), plus the
 * release-readiness result. Never counts as a client view.
 */
    ...
    <OpsShell
      ...
      actions={
        <>
          ...
          <Link href={`/ops/proposals/${publicId}/preview?mode=${other}`} className={`${s.btn} ${s.btnSecondary}`} style={btn}><Eye size={15} aria-hidden /> View as {other === "formal" ? "proposal" : "estimate"}</Link>
