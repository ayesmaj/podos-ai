...
/**
 * /ops/proposals/[publicId]/print — the semantic print source for the admin
 * (also what the PDF service prints). ?mode=formal|preliminary overrides the
 * stored page mode; ?screen=0 drops the on-screen frame (PDF).
 *
 * A proposal that fails validateProposalForRelease renders as a DESIGN
 * PREVIEW: real numbers, placeholder text masked, watermark on every page,
 * blockers listed in the cover banner. It can never be released in that state.
 */
...
export default async function OpsPrintPage({ params, searchParams }: { params: Promise<{ publicId: string }>; searchParams: Promise<{ mode?: string; screen?: string }> }) {
  ...
  if (!m) notFound();
  return <ProposalPrint d={m.doc} pageMode={m.pageMode} design={m.design} hash={m.hash} assets={m.assets} previewNotice={m.previewNotice} screen={sp.screen !== "0"} />;
}
