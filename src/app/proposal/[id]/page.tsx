...
/**
 * /proposal/[id] — the clean, session-gated client workspace (v1).
 *
 * No token in this URL: [id] is the estimate UUID (display-only routing), and
...
export default async function ProposalWorkspace({
  ...
  // The session is scoped to exactly one estimate; bind it to the URL so a
  // valid session for proposal A can never render at proposal B's address
  // (brief §30 cross-proposal test). Mismatch is a uniform 404.
  if (p.estimate_uuid !== id) notFound();
