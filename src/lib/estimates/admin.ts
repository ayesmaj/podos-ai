...
export const deleteOrganization = (secret: string, orgId: string, force = false) => rpcStrict<boolean>("delete_organization", { p_admin_secret: secret, p_org_id: orgId, p_force: force });
...
export const deleteProject = (secret: string, id: string, force = false) => rpcStrict<boolean>("delete_project", { p_admin_secret: secret, p_id: id, p_force: force });
...
export const deleteProposal = (secret: string, publicId: string, force = false) => rpcStrict<boolean>("delete_proposal", { p_admin_secret: secret, p_public_id: publicId, p_force: force });
...
/** Short numeric access code → normal admin session (the master secret is untouched). */
export async function adminLoginPin(pin: string, userAgent?: string): Promise<string | null> {
  const tok = await rpc<string>("admin_login_pin", { p_pin: pin, p_user_agent: userAgent ?? null });
  return typeof tok === "string" && tok.length === 64 ? tok : null;
}
export const setAdminPin = (secret: string, pin: string) => rpcStrict<boolean>("set_admin_pin", { p_admin_secret: secret, p_pin: pin });
