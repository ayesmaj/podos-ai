...
| `/ops/clients`, `/ops/projects`, `/ops/proposals` (lists) | per-row Open · Archive/Restore (clients) · Delete | delete of a record with released/signed proposals requires typing its name/number ("delete anyway" → forced delete in the DB function, audit-logged as *_force) |
| `/ops/login` | accepts the numeric access code (set in Settings → Admin access, default set by the founder) or the master secret | code is exchanged server-side for a normal 12-hour session; failures audit-logged and rate-limited |
